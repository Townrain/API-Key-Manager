...
class TestE2EWorkflow:
    ...
    def test_import_keys(self, client, tmp_path):
        ...
        keys_data = make_keys_data()

        def do_import(**kwargs):
            write_keys_file(Path(cfg["storage"]["keys_file"]), keys_data)
            return (1, 0, [])

        with patch("key_manager.web.routes.keys._app_mod.import_keys", side_effect=do_import), \
             patch("key_manager.web.routes.keys._app_mod.validate_import_path", side_effect=lambda p, d: Path(p)):
