...
class TencentHunyuanProvider(ProviderBase):
    ...
    check_model = "hunyuan-turbos-latest"
    ...
    async def check(self, client, key: str) -> CheckResult:
        ...
        try:
            resp = await client.post(
                ...
                headers=headers,
                json={"model": "hunyuan-turbos-latest", "messages": [{"role": "user", "content": "hi"}], "max_tokens": 5}
            )
        ...
        for step in token_steps:
            try:
                resp = await client.post(
                    f"{self.get_base_url()}/chat/completions",
                    headers=headers,
                    json={"model": "hunyuan-turbos-latest", "messages": [{"role": "user", "content": "hi"}], "max_tokens": step}
                )
                if resp.status_code == 200:
                    last_success = step
