...
# List keys
keys = client.keys(provider="openai", page=1, page_size=10)
print(keys)

# Check a single key
result = client.check_single()
print(result)
...
// List keys
const keys = await client.keys("openai", undefined, undefined, 1, 10);
console.log(keys);
...
try {
  const result = await client.checkSingle();
} catch (err) {
