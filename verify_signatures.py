...
async def main():
    ...
    console.print(f"  UNIQUE_SIGNATURES: {len(UNIQUE_SIGNATURES)} providers")
    ...
    console.print(f"\n[bold yellow]Missing Signatures (UNIQUE_SIGNATURES):[/bold yellow]")
    missing_count = 0
    for r in sorted(verification_results, key=lambda x: x["provider"]):
    ...
    for r in sorted(verification_results, key=lambda x: x["provider"]):
        provider = r["provider"]
        unique_sigs = set(s.lower() for s in UNIQUE_SIGNATURES.get(provider, []))
